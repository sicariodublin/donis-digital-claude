...
six industry demos, two legal pages, a Formspree-backed contact form, and a
...
/
  ...
  demos/
    ...
    gyms/                   Evolution Gym -- real client project (Bebas Neue +
                            Barlow Condensed + Inter), shown as a demo card;
                            see demos/gyms/EVOLUTION_GYM_PROJECT_PLAN.md
                            index.html, style.min.css, script.min.js,
                            preview.webp, plus client photos/logo
    beleza/                 Studio Lumière beauty & aesthetics demo (Geist +
                            Cormorant Garamond)
      index.html, style.css, script.js, preview.webp
    clinica/                Clínica Vida Plena medical clinic demo (Geist +
                            Playfair Display)
      index.html, style.css, script.js, preview.webp
```
...
- Six demos behind real URLs:
  ...
  - `/demos/gyms/` -- Evolution Gym: real client project (3 units, membership
    plans, WhatsApp-first sign-up), shown in the same grid as the others
  - `/demos/beleza/` -- Studio Lumière: beauty & aesthetics studio -- services, gallery, team, WhatsApp/Instagram booking
  - `/demos/clinica/` -- Clínica Vida Plena: multidisciplinary medical clinic -- specialties, team credentials, insurance list, booking
...
- **Real demo screenshots** live in the `#demos` grid on both EN and PT --
  `demos/{bella-roma,church,accountancy,gyms,beleza,clinica}/preview.webp`,
  lazy-loaded with explicit `width`/`height` to avoid layout shift, subtle
  zoom on hover.
